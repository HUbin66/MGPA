...
class NetworkXBackEnd(object):
    ...
    def graph_from_edgelist_new(self, edges: List,G:nx.Graph) -> NKGraph:
        """
        Given an edge list generate a graph.
        """
        new_graph = nx.Graph()
        for edge in edges:
            new_graph.add_node(edge[0],label=G.nodes()[edge[0]]['label'])
            new_graph.add_node(edge[1],label=G.nodes()[edge[1]]['label'])
