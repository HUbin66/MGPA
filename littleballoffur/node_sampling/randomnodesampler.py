...
class RandomNodeSampler(Sampler):
    ...
    def __init__(self, number_of_nodes, seed):
    ...
    def _create_initial_node_set(self, graph) -> List[int]:
        ...
        self.backend.get_neighbors(graph, v)
        while(len(sampled_nodes)<self.number_of_nodes):
            nei=self.backend.get_neighbors(graph, v)
            vc=random.choice(nei)
            sampled_nodes.append(vc)
            v=vc
